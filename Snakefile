...
FASTQ_FOLDERS=['../From_Simon/Modern/','../Palkopoulou_2015/','../Patrick_ENA_PRJEB24361/Modern/','../Patrick_ENA_PRJEB24361/Ancient/','../From_Simon/Ancient/'] ##### Set by user, should end with a '/', If you already have the Fastq files somewhere in your machine, add the link here so they don't need to be downloaded




#### REFERENCE TO MAP TO
REF_LOC='../REFERENCE_GENOME/'      ##### Set by user
REF=REF_LOC+'loxAfr3.fa' ##### Set by user
...
if os.path.exists('METADATA'): ##### Should check labels of METADATA file to see if the required ones are there and in which order
    ...
    for LINE in META_DATA_FILE:
        ...
        if len(LINE)<len(LABELS):
            print(F'ERROR!\nFound {len(LABELS)} columns for labels but row {counter} has {len(LINE)} columns for data.\nCheck your METADATA file, empty lines can cause this error.')
        ...
        SPECIES_HERE='_'.join(SPECIES_HERE.split(' '))
...
rule all:
    input:
        ...
        expand("{sample}.filtered.bai",sample=SAMPLES),
        "Samples_Renamed_Succss"
...
rule rename_files:
    ...
    run:
        for SAMPLE in SAMPLES:
            SN=SAMPLE_TO_SPECIES_ID[SAMPLE]
            shell(F"mv {SAMPLE}.filtered {SAMPLE}_{SN}.bam")
            shell(F"mv {SAMPLE}.filtered.bai {SAMPLE}_{SN}.bam.bai")
        shell('touch Samples_Renamed_Succss')
